...
def keep_only_positive_boxes(boxes, roi_batch_size=-1, across_sample=False):
    """
    ...
    positive_boxes = []
    if (not across_sample) or len(boxes) < 2:
        for boxes_per_image in boxes:
            labels = boxes_per_image.get_field("labels")
            pos_inds = labels > 0
            inds = pos_inds.nonzero().squeeze(1)
            if roi_batch_size > 0:
                if roi_batch_size < inds.shape[0]:
                    _inds = torch.randperm(inds.shape[0])[:roi_batch_size]
                    inds = inds[_inds]
