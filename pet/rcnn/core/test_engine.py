...
from pet.utils.misc import logging_rank, save_object
...
from pet.rcnn.datasets import build_dataset, evaluation, post_processing
...
def test_net(args, ind_range=None):
    ...
    all_boxes, all_segms, all_keyps, all_parss, all_pscores, all_uvs \
        = test(model, dataset, start_ind, end_ind, logger)
    if ind_range is not None:
