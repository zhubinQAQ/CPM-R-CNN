...
from pet.lib.ops.boxlist_ops import cat_boxlist, boxlist_nms, \
...
def im_detect_bbox(model, ims):
    ...
    if not cfg.MODEL.GRID_ON or cfg.TEST.BBOX_AUG.ENABLED:
...
def im_detect_parsing(model, rois, features):
    ...
    if cfg.TEST.BBOX_AUG.ENABLED and cfg.TEST.PARSING_AUG.ENABLED:
        ...
        for parsings_ts, result in zip(parsing_results, results):
