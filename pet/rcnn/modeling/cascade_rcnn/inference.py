...
from pet.utils.data.structures.boxlist_ops import boxlist_nms, boxlist_soft_nms, boxlist_box_voting
...
class PostProcessor(nn.Module):
    ...
    def forward(self, x, boxes, targets=None, iou_logits=None):
        """
        Arguments:
        ...
        for prob, boxes_per_img, image_shape in zip(
            ...
            boxlist = self.prepare_boxlist(boxes_per_img, prob, image_shape, iou_score)
    ...
    def prepare_boxlist(self, boxes, scores, image_shape, iou_score=None):
        """
        ...
        if iou_score is not None:
            iou_score = iou_score.repeat(1, 81).reshape(81, -1).T.reshape(-1)
            if cfg.CASCADE_RCNN.IOU_HELPER_MERGE:
