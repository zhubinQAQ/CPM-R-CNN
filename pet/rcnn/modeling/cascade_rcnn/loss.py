...
class CascadeRCNNLossComputation(object):
    """
    ...
    def match_targets_to_proposals(self, proposal, target):
        ...
        if cfg.CASCADE_RCNN.IOU_HELPER:
            return matched_targets, match_quality_matrix[:, pos_idxs]
        return matched_targets, None
    
    def prepare_iou_target(self):
        iou_targets = []
        for match_quality_matrix in self.match_quality_matrixs:
            assert match_quality_matrix is not None
    ...
    def __call__(self, class_logits, box_regression, iou_logits):
        """
        Computes the loss for Faster R-CNN.
        ...
        return classification_loss, box_loss, loss_iou
...
def box_loss_evaluator(idx):
    ...
    rescore_on = cfg.CASCADE_RCNN.RESCORE_ON
