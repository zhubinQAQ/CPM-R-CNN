...
def parsing_png(parsing, scores, img_info, output_folder):
    ...
    parsing_png = np.zeros((img_info["height"], img_info["width"]))
    parsing_ins = np.zeros((img_info["height"], img_info["width"]))

    _inx = scores.argsort()
    ins_id = 1
    for k in range(len(_inx)):
        if scores[_inx[k]] < 0.3:
            continue
        _parsing = parsing[_inx[k]]
        parsing_png = np.where(_parsing > 0, _parsing, parsing_png)
        parsing_ins = np.where(_parsing > 0, ins_id, parsing_ins)
        ins_id += 1

    cv2.imwrite(save_name, parsing_png.astype(np.uint8))
...
def compute_hist(predict_list, im_dir, num_parsing):
    ...
    for predict_png in tqdm(predict_list, desc='Calculating IoU ..'):
        ...
        im_scale = float(TEST_SCALE) / float(im_size_min)
...
def parsing_iou(predict_root, im_dir, num_parsing):
    ...
    hist, hist_s, hist_m, hist_l = compute_hist(predict_list, im_dir, num_parsing)
...
def eval_seg_ap(all_parsings, all_scores, im_dir, ann_fn, num_parsing):
    '''
...
def evaluate_parsing(all_results, eval_ap, num_parsing, im_dir, ann_fn, output_folder):
    ...
    _iou, _miou, _miou_s, _miou_m, _miou_l = parsing_iou(predict_dir, im_dir, num_parsing)

    parsing_result = {}
    parsing_result['mIoU'] = _miou
    parsing_result['mIoUs'] = _miou_s
    parsing_result['mIoUm'] = _miou_m
    parsing_result['mIoUl'] = _miou_l
    ...
    if eval_ap:
        all_ap_p, all_pcp = eval_seg_ap(all_results[0], all_results[1], im_dir, ann_fn, num_parsing)
